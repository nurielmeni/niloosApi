...
class Memad3Social extends \yii\bootstrap\Widget
{
    ...
    private function getSocials() {
        ...
        foreach ($this->socials as $name => $url) {
            if ($name === 'ins') {
                $res .= '<li style="width: 40px; height: 40px; background-image: linear-gradient(to right, #00c0ff 0%, #88ffff 100%); border-radius: 20px; margin: 5px;"><a style="background: url(\'/images/socialShare/instegram-white.png\') no-repeat center center; background-size: cover; margin: 5px; padding: 15px" href="' . $url . '"></a></li>';
            } else {
